package eu.mixeration.helper.module.pluginator;

import com.google.common.base.Joiner;
import eu.mixeration.helper.Helper;
import eu.mixeration.helper.module.Locale;
import eu.mixeration.helper.module.pluginator.utils.StringUtil;
...
public class Pluginator_Modules {

    public static void enable(Plugin plugin) {
        if (!plugin.getName().equalsIgnoreCase("Helper") && plugin != null && !plugin.isEnabled()) {
    ...
    public static String load(String name) {
        ...
        if (!pluginDir.isDirectory()) {
            return ChatColor.translateAlternateColorCodes('&', Locale.PLUGINATOR_DIRECTORY);
        }
        ...
        if (!pluginFile.isFile()) {
            for (File f : pluginDir.listFiles()) {
                if (f.getName().endsWith(".jar")) {
                    try {
                        PluginDescriptionFile desc = Helper.get().getPluginLoader().getPluginDescription(f);
                        if (desc.getName().equalsIgnoreCase(name)) {
                            pluginFile = f;
                            break;
                        }
                    } catch (InvalidDescriptionException e) {
                        return ChatColor.translateAlternateColorCodes('&', Locale.PLUGINATOR_CANNOTFIND);
                    }
                }
            }
        }
        ...
        try {
            ...
            e.printStackTrace();
            return ChatColor.translateAlternateColorCodes('&', Locale.PLUGINATOR_INVAILDDESC);
        } catch (InvalidPluginException e) {
            e.printStackTrace();
            return ChatColor.translateAlternateColorCodes('&', Locale.PLUGINATOR_INVAILDPLUGIN);
        }

        target.onLoad();
        Bukkit.getPluginManager().enablePlugin(target);

        return ChatColor.translateAlternateColorCodes('&', Locale.PLUGINATOR_LOADED.replace("<plugin>", target.getName()));
    ...
    public static void reload(Plugin plugin) {
        if (!plugin.getName().equalsIgnoreCase("Helper") && plugin != null) {
            unload(plugin);
            load(plugin);
        }
    }
    ...
    public static String unload(Plugin plugin) {
        ...
        System.gc();
        return ChatColor.translateAlternateColorCodes('&', Locale.PLUGINATOR_UNLOADED.replace("<plugin>", name));
